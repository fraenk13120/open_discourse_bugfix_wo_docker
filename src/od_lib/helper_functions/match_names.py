...
def insert_politician_id_into_speech_content(
    ...
    for index, row in df.iterrows():
        if row["politician_id"] != -1 and row["faction_id"] == -1:
            politician = politicians[politicians["ui"] == row["politician_id"]]
            if not politician.empty and politician["faction_id"].iloc[0] != -1:
                df.at[index, "faction_id"] = politician["faction_id"].iloc[0]
