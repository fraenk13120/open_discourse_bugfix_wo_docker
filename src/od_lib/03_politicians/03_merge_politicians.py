...
print(f"MPS DataFrame: {len(mps)} Einträge")
print(f"MGS DataFrame: {len(mgs)} Einträge")
print("MPS Spalten:", mps.columns.tolist())
print("MGS Spalten:", mgs.columns.tolist())
if len(mps) > 0:
    print("MPS Beispieldaten:", mps.iloc[0])
...
politicians["first_name"] = politicians["first_name"].astype(str).str.replace("-", " ", regex=False)
...
for (
    ...
    if faction_abbrev:
        faction_results = factions.loc[factions["abbreviation"] == faction_abbrev, "id"]
        if not faction_results.empty:
            faction_match = int(faction_results.iloc[0])
        else:
            faction_match = -1
    else:
        faction_match = -1
    ...
    if len(possible_matches) == 1:
        ...
        if len(possible_matches) == 1:
            ...
            ui_list = politicians["ui"].tolist()
            if ui_list:
                ui_temp = max(ui_list) + 1
            else:
                ui_temp = 1  # Oder einen anderen geeigneten Startwert
